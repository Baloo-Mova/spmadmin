...
class CommandController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($statusAdmin->status == 'SPAM') {
            ...
            try {
                ...
                if (File::exists(storage_path('app') . '/' . config('config.emails.mails') . '/' . $files[2])) {
                    ...
                    copy(storage_path('app') . '/' . config('config.emails.mails') . '/' . $files[2],
                        storage_path('app') . '/' . config('config.emails.go_mails') . '/' . $filename);
            ...
            if (count($files) > 2) {
                $result[] = url('download/' . config('config.fromname') . '/' . $files[rand(2, count($files) - 1)]);
            } else {
                $result[] = "";
            }
            ...
            if ($settings->send_attach != 0) {
                ...
                if (count($files) > 2) {
                    $result[] = url('download/' . config('config.attach') . '/' . $files[rand(2, count($files) - 1)]);
                } else {
                    $result[] = "";
                }
            }
        ...
        if ($statusAdmin->status == 'SMTPCHECK') {
            ...
            $smtpListBotCount = smtplistpiece::where('botid', '<>',
                '')->select(DB::raw('count(distinct botid) as count'))->first();
